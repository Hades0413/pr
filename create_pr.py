...
def create_local_branch(branch_name):
    ...
    # Asegurarse de agregar todos los cambios al índice
    subprocess.run(["git", "add", f"{branch_name}.txt"], check=True)
    subprocess.run(["git", "add", "."], check=True)  # Agrega todos los cambios y archivos sin seguimiento
...
# Función para eliminar archivos .txt generados
def delete_txt_files(start=1, end=50):
    for i in range(start, end + 1):
        txt_file = f"feature-branch-{YEAR}-{MONTH}-{i}.txt"
        if os.path.exists(txt_file):
            print(f"Eliminando archivo {txt_file}...")
            os.remove(txt_file)
...
def create_multiple_prs(num_prs=50):
    ...
    delete_txt_files(1, num_prs)

# Ejecutar la creación de PRs
if __name__ == "__main__":
    create_multiple_prs(2)  # Cambia 2 a la cantidad de PRs que deseas crear
